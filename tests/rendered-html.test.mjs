...
test("server-renders the complete academic personal homepage", async () => {
  ...
  assert.match(html, /src="\/dheepak-karan\.jpg"[^>]*alt="Dheepak Karan"/i);
  ...
  assert.match(html, /View all[\s\S]{0,40}6[\s\S]{0,40}projects/i);
  ...
  assert.doesNotMatch(html, /GitHub profile|LinkedIn profile/i);
  assert.match(html, /My full name is/i);
  assert.match(html, /Dheepak Karan<br\/>Elumalai Santhakumari/i);
  assert.doesNotMatch(html, /dheepak-greeting-preview\.gif|multilingual greeting/i);
  assert.match(html, /href="mailto:elumalaisanthakuma\.d@northeastern\.edu"/i);
  assert.doesNotMatch(html, /Let(?:&apos;|')s connect|email-context-preview|Have a research or teaching opportunity/i);
  ...
  assert.match(html, /Last updated August 2026 · Interface inspired by/i);
  assert.match(html, /Prof\. Weiyan Shi(?:&apos;|')s portfolio/i);
  assert.doesNotMatch(html, /Weiyan Shi(?:&apos;|')s academic portfolio/i);
  assert.match(html, /href="https:\/\/wyshi\.github\.io\/index\.html"/i);
  assert.doesNotMatch(html, />Contact<\/a>/i);
...
test("server-renders the professor-facing projects page", async () => {
  ...
  assert.match(html, /Latest activity ·/i);
  assert.match(html, /<time dateTime="\d{4}-\d{2}-\d{2}">[A-Z][a-z]{2} \d{1,2}, \d{4}<\/time>/);
  assert.match(html, /Northeastern University/i);
  ...
  assert.match(html, /Multi-Camera BEV Perception/i);
  assert.match(html, /41\.1 FPS TensorRT FP32/i);
  assert.match(html, /24\.34 ms/i);
  assert.match(html, /href="https:\/\/github\.com\/dheepakkaran\/multi-camera-bev"/i);
  assert.match(html, /xG from Words/i);
  assert.match(html, /0\.7826 AUC from commentary/i);
  assert.match(html, /90\.6%/i);
  assert.match(html, /87,980 shots/i);
  assert.match(html, /href="https:\/\/github\.com\/dheepakkaran\/xg-from-words"/i);
  assert.ok(html.indexOf("Multi-Camera BEV Perception") < html.indexOf("xG from Words"));
  assert.ok(html.indexOf("xG from Words") < html.indexOf("Multilingual LLM Fine-Tuning"));
  assert.ok(html.indexOf("Multilingual LLM Fine-Tuning") < html.indexOf("FairShare-WiFi"));
  assert.ok(html.indexOf("FairShare-WiFi") < html.indexOf("PLC Stator Water Cooling"));
  assert.ok(html.indexOf("PLC Stator Water Cooling") < html.indexOf("Petrol-to-Electric Vehicle Conversion"));
});
...
test("server-renders the dedicated academics page", async () => {
  ...
  assert.match(html, /Relevant Undergraduate Coursework/i);
  assert.match(html, /Computing &amp; embedded systems/i);
  assert.match(html, /GE8151/i);
  assert.match(html, /Problem Solving and Python Programming \(B\+\)/i);
  assert.match(html, /Object Oriented Programming \(A\+\)/i);
  assert.match(html, /Introduction to C Programming \(O\)/i);
  assert.match(html, /Microprocessors and Microcontrollers \(A\+\)/i);
  assert.match(html, /Embedded Systems \(O\)/i);
  assert.match(html, /Cloud Computing \(A\+\)/i);
  assert.match(html, /Electronics &amp; circuits/i);
  assert.match(html, /EE8251/i);
  assert.match(html, /Circuit Theory \(B\)/i);
  assert.match(html, /EC8353/i);
  assert.match(html, /Electron Devices and Circuits \(B\)/i);
  assert.match(html, /EE8461/i);
  assert.match(html, /Linear and Digital Integrated Circuits Laboratory \(O\)/i);
  assert.match(html, /Signals, control &amp; computation/i);
  assert.match(html, /Digital Signal Processing \(A\+\)/i);
  assert.match(html, /Control Systems \(B\+\)/i);
  assert.match(html, /Electrical energy &amp; automation/i);
  assert.match(html, /Power Electronics \(A\+\)/i);
  assert.match(html, /Power System Operation and Control \(O\)/i);
  assert.match(html, /Renewable Energy Systems \(A\+\)/i);
  assert.match(html, /Teaching &amp; Academic Service/i);
  ...
  assert.ok(html.indexOf("Graduate Coursework") < html.indexOf("Relevant Undergraduate Coursework"));
  assert.ok(html.indexOf("Relevant Undergraduate Coursework") < html.indexOf("Teaching &amp; Academic Service"));
  assert.ok(html.indexOf("Teaching &amp; Academic Service") < html.indexOf("<h2>Certifications</h2>"));
  assert.ok(html.indexOf("<h2>Certifications</h2>") < html.indexOf("Algorithm &amp; Problem-Solving Practice"));
...
test("ships the professor-focused academic layout, metadata and accessibility fallbacks", async () => {
  ...
  assert.match(profileHeader, /academic-avatar-static/);
  assert.doesNotMatch(profileHeader, /AnimatedAvatar|dheepak-greeting-preview|academic-avatar-greeting/);
  ...
  assert.doesNotMatch(profileHeader, /EmailPreviewLink|EmailHoverLink|email-(?:context|hover)|Let&apos;s connect/);
  assert.doesNotMatch(profileHeader, /github-profile-preview|linkedin-profile-preview|ProfilePreviewLink/);
  ...
  assert.match(academicsPage, /undergraduate-course-code/);
  ...
  assert.match(contributions, /getLatestActivity/);
  ...
  assert.doesNotMatch(css, /academic-avatar-(?:greeting|animation)|profile-preview-card/i);
  ...
  assert.match(css, /a:hover\s*\{[^}]*color:\s*#c8102e[^}]*text-decoration:\s*underline/is);
  assert.doesNotMatch(css, /email-(?:flip|hover|context)/i);
