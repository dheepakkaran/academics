...
test("server-renders the complete academic personal homepage", async () => {
  ...
  assert.match(html, /15-year-old monolith/i);
  assert.match(html, /2,000 agents/i);
  assert.match(html, /1\.2 s p95 to 700 ms/i);
  assert.match(html, /N\+1 query pattern/i);
  assert.match(html, /Apache Kafka and AWS SQS/i);
  assert.match(html, /dead-letter queue/i);
  assert.match(html, /80% line-coverage gate/i);
  assert.match(html, /120 JUnit and pytest tests/i);
  assert.match(html, /Dockerized FastAPI endpoints/i);
